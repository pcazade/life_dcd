...
DCD::DCD(const char dcdName[])
{
  dcdf.exceptions(std::ifstream::failbit);
  try
  {
    dcdf.open(dcdName,ios::in|ios::binary);
  }
  catch(std::ifstream::failure e)
  {
    cerr << "Exception opening/reading file '" << dcdName << "' : " << std::endl;
...
DCD::DCD(const char dcdName[],const bool headPrint)
{
  dcdf.exceptions(std::ifstream::failbit);
  try
  {
...
float* DCD::getZ() const {
    return Z;
}

float* DCD::getY() const {
    return Y;
}

float* DCD::getX() const {
    return X;
}
